local function MyRoutine()
	...
	local function DRST()
		-- Near Call of the Wild 4s tick window (~0.5s before a tick): only allow Bestial Wrath
		...
		-- Bestial Wrath on cooldown, hold up to 30s to sync with Call of the Wild
		...
		-- Cobra Shot filler, but not during Call of the Wild
	...
	local function DRCleave()
		...
		-- Bestial Wrath on cooldown, hold up to 30s to sync with Call of the Wild (CDs only)
		...
		-- Barbed Shot to prevent Frenzy from falling or if near full charge; avoid within ~0.5s of CotW tick
		...
		-- Multi-Shot to apply/maintain Beast Cleave; do not refresh earlier than ~0.4s; avoid if Black Arrow ready
		...
		-- Call of the Wild when CDs toggled
		...
		-- Barbed Shot if charges_fractional >= Kill Command; avoid near tick
